...
\begin{figure}[H] 
  \centering
  \includegraphics[width=0.8\columnwidth]{xlore-architecture}
  \caption{XLore系统架构图}
...
\begin{figure}[H] 
  \centering
  \begin{subfigure}{3cm}
    \includegraphics[height=5cm]{xlore-visualization-instance}
    \caption{实例可视化例子(阿甘正传)}
  %\label{fig:xlore-visualization-instance}
  \end{subfigure}
  \hspace{4em}%
  \begin{subfigure}{0.8\textwidth}
    \includegraphics[height=5cm]{xlore-visualization-concept}
...
\begin{figure}[H] 
  \centering
  \includegraphics[width=0.8\columnwidth]{entity-linking-architecture}
...
在抽取过程中，我们对名称－实体的出现频数进行了记录，用于之后的排歧工作，将会在之后详细说明。以\textit{名称 制表符 实体知识库uri 制表符 出现频数}为格式存储数据，并以名称为索引，将数据导入到MySQL数据库中，便于查询。
...
通用领域名称查找功能，应用在xxx系统，分析新闻文本，提供实体信息。如图\ref{fig:el-news}所示：
\begin{figure}[H] 
  \centering
  \includegraphics[width=0.8\columnwidth]{el-news}
  \caption{XLore实体链接接口在新闻上的应用}
  \label{fig:el-news}
\end{figure}
